...
mod ast;
use ast::{Name, Expr, Altn, Defn, Lambda, Module};
...
impl State {
  ...
  fn enter_main() -> Self {
    let expr = Expr::Ap {
        fun: Box::new(Expr::Global{name: String::from("main")}),
  ...
  fn step(&mut self, module: &Module, externals: &Externals) {
    ...
    let new_ctrl = match old_ctrl {
      Ctrl::Evaluating=>
        panic!("Control was not update after last step"),

      Ctrl::Expr(Expr::Local{name}) => {
        let v = self.env.get(&name).expect(&format!("Unknown local: {}", name));
        Ctrl::Value(v.clone())
      },
      Ctrl::Expr(Expr::Global{name}) => {
        let lam = module.get(&name).expect(&format!("Unknown global: {}", name));
        Ctrl::from_prim(Prim::Global(name, lam.clone()), lam.binds.len())
      },
      Ctrl::Expr(Expr::External{name}) => {
        let ext = externals.get(&name).expect(&format!("Unknown external: {}", name));
      ...
      Ctrl::Value(Value::PAP(prim, args, 0)) =>
        match prim {
          ...
          Prim::External(_name, ext) =>
            Ctrl::Value((ext.run)(args.into_iter().collect())),
          Prim::Pack(tag, _arity) =>
            Ctrl::Value(Value::Pack(tag, args.into_iter().collect()))
        },
...
fn externals() -> Externals {
  let add_ext =
    External {
      arity: 2,
      run: |args| { let (x, y) = args_i64_i64(args); Value::Num(x+y) }
    };
  let sub_ext =
    External {
      arity: 2,
  ...
  let eq_ext =
    External {
      arity: 2,
      run: |args| { let (x, y) = args_i64_i64(args); Value::mk_bool(x==y) }
    };
  let le_ext =
    External {
      arity: 2,
  ...
  let puti_ext =
    External {
      arity: 1,
      run: |args| { println!("{}", args_i64(args)); Value::mk_unit() }
    };
  let putc_ext =
    External {
      arity: 1,
      run: |args| { print!("{}", args_i64(args) as u8 as char); Value::mk_unit() }
    };
  let geti_ext =
    External {
      arity: 1,
